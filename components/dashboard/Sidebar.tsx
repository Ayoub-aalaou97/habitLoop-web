...
function iconClass(active: boolean) {
  return active ? "text-brand-soft" : "text-text-dim";
}
...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`sidebar fixed inset-y-0 left-0 z-30 hidden h-dvh flex-col border-r border-border-soft bg-bg-sidebar py-4 transition-[width] duration-200 lg:flex ${widthClass} ${
      ...
      >
        ...
        >
          ...
          >
            <div className="relative h-8 w-8 flex-none rounded-full">
              ...
              />
              <div className="absolute inset-[4px] rounded-full bg-bg-sidebar" />
              <div
          ...
          <button
            ...
            className="sidebar-toggle flex h-9 w-9 flex-none items-center justify-center rounded-lg text-text-muted transition hover:bg-bg-muted hover:text-text-heading"
          >
            <PanelLeftIcon />
          </button>
        </div>
            ...
            return (
              <Link
                ...
                className={`nav-item group flex items-center rounded-lg transition ${
                  ...
                  active
                    ? "bg-[rgba(111,123,255,0.14)] text-text-heading"
        ...
        <div className="mt-3 border-t border-border-soft pt-3">
          {showBestMonth && !collapsed ? (
            <div className="mb-3 rounded-2xl border border-border-soft bg-bg-elevated p-[15px]">
              <div className="mb-2 font-mono text-[10px] font-semibold tracking-[0.1em] text-text-dim">
                BEST MONTH
              </div>
              <div className="mb-0.5 text-[19px] font-bold text-text-body">
                {bestMonth}
              </div>
          ...
          {showNextBadge && nextBadge && !collapsed ? (
            <div className="mb-3 rounded-2xl border border-border-soft bg-bg-elevated p-[15px]">
              <div className="mb-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-text-dim">
                NEXT UP
              </div>
              <div className="mb-2.5 flex items-center gap-2.5">
                <div className="flex h-[38px] w-[38px] flex-none items-center justify-center rounded-full border-[1.5px] border-dashed border-border bg-bg-muted font-mono text-[12px] font-bold text-text-dim">
                  {nextBadge.mark}
                </div>
                <div className="min-w-0">
                  <div className="truncate text-[13px] font-bold text-text-body">
                    {nextBadge.label}
                  </div>
              ...
              <div className="h-[5px] overflow-hidden rounded-[3px] bg-bg-muted">
                <div
          ...
          {showQuietHours && quietHoursLabel && !collapsed ? (
            <div className="mb-3 rounded-2xl border border-border-soft bg-bg-elevated p-[15px]">
              ...
              <div className="mb-1.5 font-mono text-[15px] font-bold text-text-body">
                {quietHoursLabel}
              </div>
              <div className="text-[11px] font-medium text-text-muted">
          ...
          <button
            ...
            onClick={onLogout}
            title="Log out"
            className={`mt-1 flex items-center rounded-lg text-[13px] font-medium text-text-muted transition hover:bg-bg-muted hover:text-text-heading ${
              collapsed
